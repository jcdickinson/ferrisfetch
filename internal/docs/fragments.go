...
func implsFragment(typeData json.RawMessage, crate *RustdocCrate, item *RustdocItem, crateName, version string) *Fragment {
	...
	for _, implID := range t.Impls {
		...
		methods := listMethodSummaries(impl.Items, crate)
		if len(methods) == 0 {
			continue
		}
...
// listMethodSummaries returns brief method info (name + first line of docs) for impl block listings.
func listMethodSummaries(itemIDs []int, crate *RustdocCrate) []methodSummary {
	...
	for _, id := range itemIDs {
		...
		methods = append(methods, methodSummary{name: *methodItem.Name, docs: docs})
	}
	return methods
}
